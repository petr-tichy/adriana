class Schedule < ActiveRecord::Base
  ...
  attr_accessible :id,:graph_name, :mode, :cron, :main,:settings_server_id,:gooddata_schedule,:gooddata_process,:r_project,:is_deleted,:updated_by,:max_number_of_errors
  ...
  def self.get_public_attributes
    ["graph_name","mode","cron","is_deleted","main","setting_server_id","gooddata_schedule","gooddata_process","max_number_of_errors"]
  end
  ...
  def self.default
    ...
    select("schedule.*,running_executions.status,running_executions.event_start,running_executions.event_end,settings_server.name").joins(:project).joins(:contract).includes(:running_executions).joins(:settings_server).where("schedule.is_deleted = ?",false)
  end
