class Schedule < ActiveRecord::Base
  ...
  def self.default
    ...
    select("schedule.*,e.status,e.event_start,e.event_end,project.name as project_name,project.contract_id,settings_server.name").joins(:settings_server).joins(:project).joins("LEFT OUTER JOIN contract ON contract.id = project.contract_id").joins("LEFT OUTER JOIN execution_log e ON e.r_schedule = schedule.id").where("NOT EXISTS (SELECT e1.id FROM execution_log e1 WHERE e1.r_schedule = e.r_schedule and e1.id > e.id) and schedule.is_deleted = 'f'")
