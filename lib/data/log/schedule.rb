module SLAWatcher
  class Schedule < ActiveRecord::Base
    ...
    def self.load_schedules_of_live_projects
      select("schedule.id as id,ss.server_type as server_type,schedule.cron as cron").joins("INNER JOIN log2.project p ON r_project = p.project_pid").joins("INNER JOIN log2.settings_server ss ON ss.id = schedule.settings_server_id").where("p.status = ? and schedule.is_deleted = ? and p.contract_id IS NULL","Live",false)
    end
